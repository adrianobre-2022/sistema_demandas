...
# 📄 FUNÇÃO EXTERNA DE GERAÇÃO DO PDF
...
def gerar_pdf_demandas(df_aba):
    try:
        ...
        pdf.cell(200, 10, txt="Relatorio de Demandas", ln=1, align="C")
        for _, r in df_aba.iterrows():
            txt_l = f"- Falta: {r['O que Falta']} | Local: {r['Local/Referência']}"
...
def desenhar_morador(s_l, nm, num_aba, dict_liberados, supabase, loja_alvo):
    ...
    is_dono_vazio = (sub_local == loja_alvo) and not st.session_state.get(
...
def renderizar(supabase):
    ...
    # ⬅️ BARRA NATIVA RESPONSIVA DE DESCONEXÃO
    ...
    # 🛠️ AMBIENTE EXCLUSIVO DO ADMIN MESTRE
    if p_cli == "admin":
        st.markdown(
        ...
        with st.form(key="form_admin_mestre_cad", clear_on_submit=True):
            ...
            if st.form_submit_button("💼 Cadastrar Lojista"):
                if nome_novo and regiao_novo:
                    try:
                        novo_registro = supabase.table("clientes_b2b").insert({"nome_estabelecimento": nome_novo.strip().title(), "perfil_segmento": perfil_novo, "regiao_atuacao": regiao_novo.strip(
            ...
            if resposta_clientes.data:
                for cli in resposta_clientes.data:
                    ...
                    with st.expander(f"🏢 {cli['nome_estabelecimento']}"):
                        ...
                        c_pdf = st.checkbox("Emissão de Relatórios PDF", value=cli.get(
        ...
        if st.session_state.dados_grafico is not None and not st.session_state.dados_grafico.empty:
            ...
            for num_aba, n_aba_atv in enumerate(n_abas):
                with abas_st[num_aba]:
                    ...
                    dict_liberados = st.session_state.get(
                        "recursos_liberados", {})

                    if is_rev and not (dict_liberados.get("reverso", True) if isinstance(dict_liberados, dict) else True):
                        st.warning("🔒 Suspensa.")
                        continue
                    ...
                    if not df_f_aba.empty:
                        ...
                        if dict_liberados.get("pdf", True) if isinstance(dict_liberados, dict) else True:
