...
# --- TELA: PAINEL DE DECISÃO ESTRATÉGICA (B2B) ---
    ...
    # --- INTERFACE MASTER ISOLADA: PAINEL ADMINISTRATIVO SECRETO (GERADOR DE TOKENS UUID) ---
    if st.session_state.perfil_cliente == "admin":
        ...
        st.markdown(
            "##### *Preencha os campos obrigatórios para emissão automática de tokens UUID na nuvem.*")

        with st.form(key="form_cadastro_b2b_admin", clear_on_submit=True):
            nome_novo_comercio = st.text_input("Nome do Estabelecimento Comercial / Cliente Real:",
                                               placeholder="Ex: Supermercado Xavier, Petshop Bairro Alto, Jornal da Cidade...")
    ...
    # --- FLUXO DOS DEMAIS PERFIS CORPORATIVOS (CLIENTES COMERCIAIS / IMPRENSA / GESTORES) ---
    else:
                ...
                if not df_filtrado.empty:
                    ...
                    if espectador_analitico:
                        ...
                        for indice, dynamic_line in df_analitico.iterrows():
                            ...
                            st.write(
                                "📍 **Localização das Reclamações Coletadas:**")
