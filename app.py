...
# --- TELA: PAINEL DO COMERCIANTE / GESTOR ---
    ...
    st.markdown(
        "##### *Tenha o produto, marca ou serviço que o seu cliente deseja na prateleira ou região.*")
    st.write("---")

    filtro_frente = st.selectbox(
        label="Selecione a Frente de Inteligência que deseja analisar:",
        options=["Todas as Carências",
                 "Apenas Produtos/Marcas (Varejo)", "Oportunidades de Novos Negócios (Serviços)", "Infraestrutura Urbana (Setor Público)"],
    ...
    if st.button("Buscar Oportunidades Ocultas", use_container_width=True, key="btn_buscar"):
        try:
            resposta = supabase.table("relatos_escassez").select(
                "item_solicitado, tipo_carencia, locais_destino(nome_exibicao, regiao_cidade)").execute()
            ...
            if resposta.data:
                ...
                if dados_limpos:
                    ...
                    if not df.empty:
                        st.write(
                            f"📈 Foram encontrados **{len(df)}** registros mapeados:")
                        ...
                        st.info(
                            "ℹ️ Nenhum registro encontrado para os filtros selecionados.")
                else:
                    st.info(
