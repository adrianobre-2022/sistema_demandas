...
# --- CUSTOMIZAÇÃO ESTÉTICA PREMIUM (TRAVA TOTAL DE VERDE E CONTRASTE) ---
st.markdown("""
    <style>
    .stApp {
    ...
    /* FORÇA O VERDE CORPORATIVO E TEXTO PRETO EM ABSOLUTAMENTE TODOS OS BOTÕES DO APP */
...
if st.session_state.tela_atual == "home":
    # --- FACHADA PADRONIZADA: APENAS A PRIMEIRA LETRA EM MAIÚSCULA COM INTERROGAÇÃO ---
...
# --- TELA: FORMULÁRIO DO CONSUMIDOR ---
    ...
    if st.session_state.aba_consumidor == "menu_triagem":
        ...
        # TÍTULOS CURTOS CORRIGIDOS: Sem a palavra "Formulário", com interrogação e minúsculas
...
# --- TELA: PAINEL DO COMERCIANTE / GESTOR ---
    ...
    if st.button("Buscar Oportunidades Ocultas", use_container_width=True, key="btn_buscar"):
        ...
        try:
            ...
            # --- TRAVA DE SEGURANÇA CONTRA BANCO VAZIO: Injeta dados fictícios de teste automáticos se o banco limpar ---
            if not dados_limpos:
                dados_limpos = [
                    ...
                    {"ID": 993, "O que Falta": "Sapataria Rápida", "Categoria": "Serviço Local / Novo Estabelecimento",
                        "Local/Referência": "Rua das Palmeiras", "Cidade": "São Paulo", "Dias": 8, "Observação": "Falta comércio de sapataria por aqui."},
                    {"ID": 994, "O que Falta": "Lavanderia Expressa", "Categoria": "Serviço Local / Novo Estabelecimento", "Local/Referência": "Praça Central",
                        "Cidade": "São Paulo", "Dias": 15, "Observação": "Bairro cheio de prédios novos sem lavanderia por perto."},
                    {"ID": 995, "O que Falta": "Manutenção de Iluminação", "Categoria": "Serviço Público / Infraestrutura", "Local/Referência": "Rua 3 número 40",
                        "Cidade": "São Paulo", "Dias": 2, "Observação": "Posto com lâmpada piscando, gerando escuridão extrema."}
                ]
    ...
    if st.session_state.busca_ativa and st.session_state.dados_grafico is not None:
        ...
        if not df.empty:
            ...
            if not df_filtrado.empty:
                # --- AGROUPAMENTO DINÂMICO POR PERFIL B2B CORRIGIDO ---
                if st.session_state.perfil_cliente == "comerciante":
