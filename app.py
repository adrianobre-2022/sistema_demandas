        ...
        if botao_enviar and item_solicitado and local_ocorrencia:
            try:
                ...
                if local_id:
                    ...
                    import time
                    time.sleep(1.2)
                    st.session_state.aba_consumidor = "menu_triagem"; st.session_state.tela_atual = "home"; st.rerun()
...
# --- TELA: AUTENTICAÇÃO POR TOKEN (REATIVAÇÃO DO ENTER) ---
    ...
    if st.button("⬅️ Voltar ao Menu Principal", key="btn_voltar_aut"):
        st.session_state.tela_atual = "home"
        st.session_state.token_valido = False; st.rerun()
    st.markdown("<h1 style='text-align: center; font-weight: 900; margin-bottom: 0px;'>🔍 E o que falta?</h1>",
                unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; font-size: 16px; font-style: italic; color: #aaaaaa; margin-top: 5px; margin-bottom: 25px;'>O termômetro de carências da nossa região.</p>", unsafe_allow_html=True)
    st.write("---")
    ...
    if botao_validar and token_inserido:
        ...
        if token_limpo == "COMERCIO10":
            ...
            st.session_state.perfil_cliente = "comerciante"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "SAUDE20":
            st.session_state.token_valido = True; st.session_state.perfil_cliente = "saude"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "PET30":
            st.session_state.token_valido = True
            st.session_state.perfil_cliente = "petshop"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "BELEZA40":
            st.session_state.token_valido = True; st.session_state.perfil_cliente = "beleza"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "INVEST20":
            st.session_state.token_valido = True
            st.session_state.perfil_cliente = "investidor"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "GESTOR30":
            st.session_state.token_valido = True; st.session_state.perfil_cliente = "gestor"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "MIDIA40":
            st.session_state.token_valido = True
            st.session_state.perfil_cliente = "jornalista"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo == "ADMIN99":
            st.session_state.token_valido = True; st.session_state.perfil_cliente = "admin"; st.session_state.regiao_cliente = "São Paulo/SP"; st.session_state.tela_atual = "comerciante"; st.rerun()
        elif token_limpo != "":
            try:
...
# --- TELA: PAINEL DE DECISÃO ESTRATÉGICA (B2B) ---
    ...
    if st.session_state.perfil_cliente != "admin":
        regioes_disponiveis_teste = ["São Paulo/SP", "Carapicuíba/SP", "Osasco/SP", "Barueri/SP"]
        regiao_padrao_index = regioes_disponiveis_teste.index(st.session_state.regiao_cliente) if st.session_state.regiao_cliente in regioes_disponiveis_teste else 0
        regiao_selecionada_painel = st.selectbox("📍 Analisar Região / Cidade (Filtro Dinâmico):", options=regioes_disponiveis_teste, index=regiao_padrao_index, key="select_filtro_geografico_b2b")
    ...
    if st.session_state.perfil_cliente == "admin":
        ...
        with st.form(key=id_formulario_admin, clear_on_submit=True):
            ...
            regiao_novo_comercio = st.text_input("Região/Cidade de Atuação:", placeholder="Ex: São Paulo/SP...")
            st.write("")
            ...
            if resposta_clientes.data and len(resposta_clientes.data) > 0:
                for cli in resposta_clientes.data:
                    ...
                    with st.expander(f"🏢 {cli['nome_estabelecimento']} ({cli['regiao_atuacao']})"):
                        # Campo de Texto Seguro e Copiável para o Token UUID Inteiro
            ...
            if sugestoes_brutas.data and len(sugestoes_brutas.data) > 0:
                for sug in sugestoes_brutas.data:
                    ...
                    with st.expander(f"📥 Termo Coletado: \"{item_sug}\""):
                        nicho_homologado = st.selectbox("Vincular ao Segmento Oficial:", ["Supermercado", "Saude", "Petshop", "Beleza"], key=f"sel_nicho_{id_sug}")
    ...
    if st.session_state.perfil_cliente != "admin":
        if not st.session_state.busca_ativa or st.session_state.dados_grafico is None:
            ...
            try:
                regiao_alvo = st.session_state.get("regiao_cliente", "São Paulo/SP")
                cidade_filtro = regiao_alvo.split("-")[0].strip() if "-" in regiao_alvo else regiao_alvo.strip()
                resposta = supabase.table("relatos_escassez").select("id, item_solicitado, tipo_carencia, data_registro, status, observacao_detalhe, sub_segmento, pegada_digital, contato_aviso, locais_destino(nome_exibicao, regiao_cidade)").execute()
                ...
                if resposta.data and len(resposta.data) > 0:
                    for registro in resposta.data:
                        if registro.get("locais_destino"):
                            ...
                            if cidade_filtro.lower() not in loc_cidade.lower(): continue
                            sub_seg = str(registro.get("sub_segmento", "Geral")).strip()
                            cat_bruta = str(registro.get("tipo_carencia", "Produto / Marca")).strip()
                            idade_dias = max(0, (agora - datetime.datetime.fromisoformat(registro.get("data_registro").replace("Z", "+00:00"))).days) if registro.get("data_registro") else 0
                            ...
                            dados_limpos.append({"ID": registro["id"], "O que Falta": registro["item_solicitado"].strip().title(), "Categoria": categoria_limpa, "Local/Referência": nome_local_exibicao, "Cidade": loc_cidade, "Dias": idade_dias, "Observação": registro.get("observacao_detalhe") or "Sem detalhes.", "SubSegmento": sub_seg, "Pegada": registro.get("pegada_digital") or f"anon_{registro['id']}", "Contato": registro.get("contato_aviso") or ""})
                
                dados_mock = [
                    {"ID": 991, "O que Falta": "Leite Desnatado Parmalat 1L", "Categoria": "Produto / Marca", "Local/Referência": "Mercadinho Do Bairro", "Cidade": "São Paulo/SP - Centro", "Dias": 4, "Observação": "Falta toda quarta.", "SubSegmento": "Supermercado", "Pegada": "hash1", "Contato": "11999999999"},
                    {"ID": 994, "O que Falta": "Lavanderia Expressa Auto-Servico", "Categoria": "Serviço Local / Novo Estabelecimento", "Local/Referência": "Avenida Das Palmeiras", "Cidade": "São Paulo/SP - Tatuapé", "Dias": 45, "Observação": "Prédios novos sem serviço.", "SubSegmento": "Investimento", "Pegada": "hash4", "Contato": ""},
                    {"ID": 997, "O que Falta": "Manutenção De Iluminação Pública", "Categoria": "Serviço Público / Infraestrutura", "Local/Referência": "Rua das Flores, 40", "Cidade": "São Paulo/SP - Centro", "Dias": 2, "Observação": "Poste apagado.", "SubSegmento": "Zeladoria", "Pegada": "hash7", "Contato": ""}
                ]
                for m in dados_mock:
                    if cidade_filtro.lower() in m["Cidade"].lower():
                        if not any(d["O que Falta"].lower() == m["O que Falta"].lower() for d in dados_limpos):
                            if st.session_state.perfil_cliente in ["comerciante", "saude", "petshop", "beleza"] and m["Local/Referência"] != loja_alvo_prioridade:
                                m_copy = m.copy(); m_copy["Local/Referência"] = "Estabelecimento Concorrente"
                                dados_limpos.append(m_copy)
                            else: dados_limpos.append(m)
                st.session_state.dados_grafico = pd.DataFrame(dados_limpos)
            except Exception as e: st.error(f"⚠️ Erro técnico de performance: {str(e)}")
                        ...
                        if not df_filtro_aba.empty:
                            ...
                            if espectador_analitico:
                                ...
                                for _, mestre_line in df_agrupado_mestre.iterrows():
                                    ...
                                    detalhes_item_limpo = detalhes_item.drop_duplicates(subset=["Cidade", "Local/Referência", "Observação"])
                            ...
                            # --- MODELO B: INTERFACE OPERACIONAL COMERCIAL (LOJISTAS) ---
                                ...
                                for _, linha in df_agrupado_mestre.iterrows():
                                    ...
                                    detalhes_item = df_filtro_aba[df_filtro_aba['O que Falta'] == item_nome]
                                    detalhes_item_limpo = detalhes_item.drop_duplicates(subset=["Cidade", "Local/Referência", "Observação", "Contato"])
                                    ...
                                    for _, sub_linha in detalhes_item_limpo.iterrows():
                                        ...
                                        st.markdown(f"{'🔥 **SEU ESTABELECIMENTO:** ' if is_dono_vazio else ('🎯 **REVERSO (Lead de Venda):** ' if is_reverso_ativa else '📍 **Captado no concorrente:** ')}{sub_local} ({sub_linha['Cidade']})")
