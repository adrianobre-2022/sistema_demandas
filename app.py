...
# --- TELA: FORMULÁRIO DO CONSUMIDOR ---
    ...
    regiao_selecionada = st.selectbox(
        label="Selecione a sua Região / Cidade para registrar a carência:",
        options=["Carapicuíba - Centro", "Carapicuíba - Aldeia", "São Paulo - Zona Oeste", "São Paulo - Zona Sul", "Rio de Janeiro - Barra da Tijuca", "Rio de Janeiro - Centro"],
        key="seletor_regiao_expandida"
    )
    st.write("---")
    ...
    if st.session_state.aba_consumidor == "menu_triagem":
        ...
        with st.form(key="formulario_dinamico_consumidor", clear_on_submit=True):
            ...
            observacao_usuario = st.text_area(label="Mais detalhes ou observações sobre o problema (Opcional):", placeholder="Ex: Detalhe o ocorrido...", key="input_obs")
        ...
        if botao_enviar:
            if item_solicitado and local_ocorrencia:
                ...
                if any(p in texto_usuario for p in palavras_ofensivas) or any(p in local_usuario for p in palavras_ofensivas) or any(p in obs_texto for p in palavras_ofensivas):
                    contem_bloqueio = True; mensagem_erro = "⚠️ O sistema identificou termos impróprios ou linguagem ofensiva."
                if any(p in texto_usuario for p in termos_politicos_proibidos) or any(p in local_usuario for p in termos_politicos_proibidos) or any(p in obs_texto for p in termos_politicos_proibidos):
                    contem_bloqueio = True; mensagem_erro = "⚠️ O portal é focado estritamente em zeladoria e carências locais."
                ...
                if not erro_detectado:
                    try:
                        texto_regiao = st.session_state.seletor_regiao_expandida
                        estado_detectado = "RJ" if "Rio" in texto_regiao else "SP"
                        local_formatado = local_ocorrencia.strip().title()
                        local_data = supabase.table("locais_destino").insert({"nome_exibicao": local_formatado, "regiao_cidade": texto_regiao, "regiao_estado": estado_detectado}).execute()
                        local_id = local_data.data["id"] if local_data.data and len(local_data.data) > 0 else None
...
# --- TELA: AUTENTICAÇÃO POR TOKEN ---
    ...
    if st.button("Validar Credenciais", use_container_width=True):
        if token_inserido == "COMERCIO10":
            ...
            st.error("❌ Token inválido ou expirado. Entre em contato com o administrador.")
# --- TELA: PAINEL DO COMERCIANTE / GESTOR ---
    ...
    if st.button("Buscar Oportunidades Ocultas", use_container_width=True, key="btn_buscar"):
        ...
        try:
            resposta = supabase.table("relatos_escassez").select("id, item_solicitado, tipo_carencia, data_registro, status, detalhes_adicionais, observacao_detalhe, sub_segmento, locais_destino(nome_exibicao, regiao_cidade)").execute()
            dados_limpos = []
            ...
            if not dados_limpos:
                dados_limpos = [
                    {"ID": 991, "O que Falta": "Leite Desnatado Integrado", "Categoria": "Produto / Marca", "Local/Referência": "Mercadinho Do Bairro", "Cidade": "São Paulo", "Dias": 4, "Observação": "Falta nas prateleiras toda quarta à tarde.", "SubSegmento": "Supermercado"},
                    {"ID": 993, "O que Falta": "Fisioterapeuta Pediátrico", "Categoria": "Serviço Local / Novo Estabelecimento", "Local/Referência": "Condomínio Novo - Bloco B", "Cidade": "São Paulo", "Dias": 5, "Observação": "Não há clínicas com essa especialidade perto.", "SubSegmento": "Saude"},
                    {"ID": 995, "O que Falta": "Ração Premium Gatos Castrados", "Categoria": "Produto / Marca", "Local/Referência": "Petshop Bairro Alto", "Cidade": "São Paulo", "Dias": 3, "Observação": "Marca X sumiu do estoque.", "SubSegmento": "Petshop"},
                    {"ID": 996, "O que Falta": "Barbearia Retrô", "Categoria": "Serviço Local / Novo Estabelecimento", "Local/Referência": "Avenida Principal 1200", "Cidade": "São Paulo", "Dias": 14, "Observação": "Homens do bairro precisam viajar ao centro para cortar cabelo.", "SubSegmento": "Beleza"},
                    {"ID": 997, "O que Falta": "Manutenção De Iluminação", "Categoria": "Serviço Público / Infraestrutura", "Local/Referência": "Rua das Flores, 40", "Cidade": "São Paulo", "Dias": 2, "Observação": "Poste apagado gerando escuridão extrema.", "SubSegmento": "Geral"}
                ]
    ...
    if st.session_state.busca_ativa and st.session_state.dados_grafico is not None:
        ...
        if not df.empty:
            ...
            if not df_filtrado.empty:
                # --- AGRUPAMENTO COMERCIAL CONSOLIDADO ---
                df_agrupado = df_filtrado.groupby(["O que Falta", "Categoria", "Cidade"]).agg(Volume_Total=("ID", "count"), Menor_Idade=("Dias", "min")).sort_values(by="Volume_Total", ascending=False).reset_index()

                st.write("---")
                st.markdown("#### 📥 Exportar Inteligência de Mercado")
                df_exportar = df_agrupado.copy()
                df_exportar.columns = ["Item", "Segmento", "Cidade", "Pedidos", "Dias"]
                ...
                estilo_titulo = ParagraphStyle('T', parent=estilos['Heading1'], fontSize=18, textColor=colors.HexColor('#00803B'), spaceAfter=15)
                estilo_texto = ParagraphStyle('X', parent=estilos['Normal'], fontSize=10, spaceAfter=20)
                
                elementos_pdf.append(Paragraph("<b>RELATÓRIO GERENCIAL - INTELIGÊNCIA DE MERCADO</b>", estilo_titulo))
                from zoneinfo import ZoneInfo
                data_hora_brasil = datetime.datetime.now(ZoneInfo("America/Sao_Paulo")).strftime('%d/%m/%Y %H:%M')
                elementos_pdf.append(Paragraph(f"Perfil: {st.session_state.perfil_cliente.upper()}<br/>Emissão: {data_hora_brasil}", estilo_texto))
                ...
                tabela_pdf.setStyle(TableStyle([
                    ...
                    ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#DDDDDD'))
                ]))
                elementos_pdf.append(tabela_pdf)
                doc.build(elementos_pdf)
                dados_pdf_final = buffer_pdf.getvalue()
                
                st.download_button(label="📥 Baixar Relatório Gerencial (PDF)", data=dados_pdf_final, file_name=f"relatorio_{st.session_state.perfil_cliente}.pdf", mime="application/pdf", key="btn_pdf_uni")
                ...
                for indice, linha in df_agrupado.iterrows():
                    ...
                    classe_tag = "tag-calor-alta" if volume >= 7 else ("tag-calor-media" if volume >= 3 else "tag-calor-baixa")
                    label_tag = f"CRÍTICA • {int(volume)} Pedidos" if volume >= 7 else (f"MODERADA • {int(volume)} Pedidos" if volume >= 3 else f"INICIAL • {int(volume)} Pedido")
                    ...
                    for _, sub_linha in detalhes_item.iterrows():
                        st.markdown(f"📍 **Local Geográfico Exato:** {sub_linha['Local/Referência']}")
                        obs_texto = str(sub_linha['Observação']).strip()
                        if obs_texto: st.info(f"💬 *Relato:* \"{obs_texto}\"")
                    ...
                    if not st.session_state[chave_confirmacao]:
                        ...
                        st.warning(f"⚠️ Confirma dar baixa em todas as {int(volume)} solicitações?")
                        ...
                        with col_b2:
                            if st.button("❌ Cancelar", key=f"btn_cancelar_{indice}"):
                                st.session_state[chave_confirmacao] = False; st.rerun()
