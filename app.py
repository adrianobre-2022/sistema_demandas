...
# --- CUSTOMIZAÇÃO ESTÉTICA PREMIUM (TRAVA TOTAL DE VERDE ESCURO E TEXTO PRETO - ACESSIBILIDADE WCAG) ---
st.markdown("""
    <style>
    .stApp {
    ...
    .stButton>button, .stFormSubmitButton>button, [data-testid="stDownloadButton"]>button {
        ...
        color: #000000 !important; /* Texto preto absoluto, elimina o efeito incandescente */
        font-weight: 900 !important; /* Negrito ultra pesado para leitura perfeita */
...
# --- TELA: FORMULÁRIO DO CONSUMIDOR ---
    ...
    if st.session_state.aba_consumidor == "menu_triagem":
        ...
        with st.form(key="formulario_dinamico_consumidor", clear_on_submit=True):
            ...
            observacao_usuario = st.text_area(label="Mais detalhes ou observações sobre o problema (Opcional):", placeholder="Ex: Detalhe o ocorrido de forma construtiva para ajudar a triagem...", key="input_obs")
        ...
        if botao_enviar:
            if item_solicitado and local_ocorrencia:
                ...
                if any(p in texto_usuario for p in termos_politicos_proibidos) or any(p in local_usuario for p in termos_politicos_proibidos) or any(p in obs_texto for p in termos_politicos_proibidos):
                    contem_bloqueio = True; mensagem_erro = "⚠️ O portal é focado estritamente em zeladoria e carências locais. Manifestações político-ideológicas nacionais (como PECs) devem ser direcionadas às ouvidorias do Senado ou Câmara."
                ...
                if not erro_detectado:
                    try:
                        ...
                        local_data = supabase.table("locais_destino").insert({"nome_exibicao": local_formatado, "regiao_cidade": "São Paulo", "regiao_estado": "SP"}).execute()
                        ...
                        if local_id:
                            ...
                            if any(p in texto_usuario for p in ["leite", "arroz", "feijão", "café", "açúcar", "refrigerante", "cerveja", "sabão"]): segmento_detectado = "Supermercado"
                            elif any(p in texto_usuario for p in ["remédio", "xarope", "fralda", "pomada", "curativo"]): segmento_detectado = "Farmácia"
                            elif any(p in texto_usuario for p in ["ração", "pet", "cachorro", "gato", "coleira"]): segmento_detectado = "Petshop"
                            elif any(p in texto_usuario for p in ["pão", "bolo", "doce", "salgado", "padaria"]): segmento_detectado = "Padaria"
...
# --- TELA: PAINEL DO COMERCIANTE / GESTOR ---
    ...
    if st.button("Buscar Oportunidades Ocultas", use_container_width=True, key="btn_buscar"):
        ...
        try:
                ...
                for registro in resposta.data:
                    if registro.get("locais_destino"):
                        ...
                        if data_str:
                            ...
                            if categoria == "Produto / Marca" and idade_dias <= 30: manter_registro = True
                            elif categoria == "Serviço Local / Novo Estabelecimento" and idade_dias <= 365: manter_registro = True
                            elif categoria == "Serviço Público / Infraestrutura" and idade_dias <= 180: manter_registro = True
                                
                            if manter_registro:
                                dados_limpos.append({
                                    "ID": registro["id"], "O que Falta": registro["item_solicitado"], "Categoria": categoria,
            ...
            # --- TRAVA DE SEGURANÇA CONTRA BANCO VAZIO: Injeta dados fictícios automáticos caso o Supabase limpe ---
            if not dados_limpos:
                dados_limpos = [
                    ...
                    {"ID": 994, "O que Falta": "Lavanderia Expressa", "Categoria": "Serviço Local / Novo Estabelecimento", "Local/Referência": "Praça Central", "Cidade": "São Paulo", "Dias": 15, "Observação": "Bairro cheio de prédios novos sem lavanderia por perto."},
    ...
    if st.session_state.busca_ativa and st.session_state.dados_grafico is not None:
        ...
        if not df.empty:
            ...
            if not df_filtrado.empty:
                ...
                if st.session_state.perfil_cliente == "comerciante":
                    df_agrupado = df_filtrado.groupby(["O que Falta", "Categoria", "Local/Referência", "Cidade", "Observação"]).agg(Volume_Pedidos=("ID", "count"), Menor_Idade=("Dias", "min")).sort_values(by="Volume_Pedidos", ascending=False).reset_index()
                else:
                    df_agrupado = df_filtrado.groupby(["O que Falta", "Categoria", "Cidade", "Observação"]).agg(Volume_Pedidos=("ID", "count"), Menor_Idade=("Dias", "min")).sort_values(by="Volume_Pedidos", ascending=False).reset_index()
                    df_agrupado["Local/Referência"] = "Mapeamento Consolidado da Região"

                st.markdown("#### 🔥 Termômetro de Demandas Reprimidas (Ranking)")
                st.write("##### *Análise em tempo real ordenada por volume de intenção de compra:*")
                for _, linha_rank in df_agrupado.iterrows():
                    ...
                    label_tag = f"CRÍTICA • {volume} Pedidos" if volume >= 7 else (f"MODERADA • {volume} Pedidos" if volume >= 3 else f"INICIAL • {volume} Pedido")
                    st.markdown(f'<div style="background-color: #1E1E1E; padding: 0.8rem; border-radius: 8px; margin-bottom: 0.5rem; border: 1px solid #333;"><span class="{classe_tag}">{label_tag}</span><b style="color: #FFFFFF; font-size: 15px;">📦 {linha_rank["O que Falta"]}</b></div>', unsafe_allow_html=True)
                
                st.write("---")
                st.markdown("#### 📥 Exportar Inteligência de Mercado")
                df_exportar = df_agrupado.copy()
                if st.session_state.perfil_cliente == "comerciante":
                    df_exportar.columns = ["Item Solicitado", "Segmento", "Ponto de Referência", "Cidade", "Detalhes/Contexto", "Volume de Pedidos", "Dias Desde o Alerta"]
                else:
                    df_exportar.columns = ["Item Solicitado", "Segmento", "Cidade", "Detalhes/Contexto", "Volume Total de Pedidos", "Dias Desde o Alerta", "Escopo"]
                ...
                estilo_titulo = ParagraphStyle('TituloPDF', parent=estilos['Heading1'], fontSize=18, textColor=colors.HexColor('#00B359'), spaceAfter=15)
                ...
                elementos_pdf.append(Paragraph(f"Frente de Análise: {st.session_state.perfil_cliente.upper()}<br/>Data de emissão: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", estilo_texto))
                ...
                tabela_pdf.setStyle(TableStyle([
                    ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#00B359')), ('TEXTCOLOR', (0,0), (-1,0), colors.white),
