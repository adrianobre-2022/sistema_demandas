...
# --- TELA: PAINEL DE DECISÃO ESTRATÉGICA (B2B) ---
    ...
    if st.button("⬅️ Sair do Painel (Logoff)", key="btn_voltar_com"):
        st.session_state.tela_atual = "home"; st.session_state.token_valido = False; st.perfil_cliente = None; st.session_state.busca_ativa = False; st.session_state.dados_grafico = None; st.rerun()
    
    st.markdown("<h1 style='text-align: center; margin-bottom: 0px !important;'>🔍 E o que falta?</h1>", unsafe_allow_html=True)
    ...
    if st.session_state.perfil_cliente == "comerciante":
        ...
        st.markdown("##### 🏪 *Nível de Acesso: Varejo Local (Foco em Gôndolas)*")
        ...
        st.markdown("##### 💼 *Nível de Acesso: Investidor e Expansão (Vazios Comerciais)*")
    elif st.session_state.perfil_cliente == "jornalista":
        opcoes_filtro = ["Infraestrutura Urbana (Setor Público)", "Oportunidades de Novos Negócios (Serviços)"]
        st.markdown("##### 📰 *Nível de Acesso: Imprensa e Jornalismo Regional (Dados Consolidados)*")
        ...
        st.markdown("##### 🏛️ *Nível de Acesso: Gestão Pública (Foco em Infraestrutura)*")
    ...
    if st.session_state.perfil_cliente == "admin":
        ...
        st.markdown("##### *Preencha os campos obrigatórios para emissão automática de tokens UUID na nuvem.*")
        ...
        with st.form(key=id_formulario_admin, clear_on_submit=True):
            nome_novo_comercio = st.text_input("Nome do Estabelecimento Comercial / Cliente Real:", placeholder="Ex: Supermercado Xavier, Petshop Bairro Alto...")
            perfil_novo_comercio = st.selectbox("Perfil de Acesso Corporativo (Nível de Filtro):", ["comerciante", "saude", "petshop", "beleza", "investidor", "gestor", "jornalista"])
            st.write("")
            botao_gerar_chave = st.form_submit_button("💼 Cadastrar Cliente e Emitir Token UUID")
            if botao_gerar_chave:
                if nome_novo_comercio:
                    try:
                        novo_registro = supabase.table("clientes_b2b").insert({"nome_estabelecimento": nome_novo_comercio.strip().title(), "perfil_segmento": perfil_novo_comercio}).execute()
                        if novo_registro.data:
                            st.success("🎉 Cliente cadastrado com sucesso absoluto na nuvem!")
                            st.info(f"🔑 **TOKEN PRIVADO GERADO:** `{novo_registro.data['token_acesso']}`")
                            st.warning("Copie o código acima e envie agora mesmo para o WhatsApp do cliente pagante.")
                ...
                if resposta.data and len(resposta.data) > 0:
                    for registro in resposta.data:
                        if registro.get("locais_destino"):
                            ...
                            if st.session_state.perfil_cliente in ["comerciante", "saude", "petshop", "beleza"] and ("Infraestrutura" in cat_bruta or "Público" in cat_bruta): continue
                            if "Marketplace" not in filtro_frente and not espectador_analitico:
                                if st.session_state.perfil_cliente == "comerciante" and sub_seg != "Supermercado": continue
                                elif st.session_state.perfil_cliente == "saude" and sub_seg != "Saude": continue
                                elif st.session_state.perfil_cliente == "petshop" and sub_seg != "Petshop": continue
                                elif st.session_state.perfil_cliente == "beleza" and sub_seg != "Beleza": continue
                            if st.session_state.perfil_cliente == "investidor" and (sub_seg != "Investimento" or "Local" not in cat_bruta): continue
                            if st.session_state.perfil_cliente == "jornalista" and (sub_seg == "Supermercado" or sub_seg == "Saude" or sub_seg == "Petshop" or sub_seg == "Beleza"): continue
                            if st.session_state.perfil_cliente == "gestor" and "Público" not in cat_bruta: continue
                            ...
                            categoria_limpa = "Serviço Público / Infraestrutura" if "Público" in cat_bruta or "Infra" in cat_bruta else ("Serviço Local / Novo Estabelecimento" if "Local" in cat_bruta or "Serviço" in cat_bruta else "Produto / Marca")
                            dados_limpos.append({"ID": registro["id"], "O que Falta": registro["item_solicitado"].strip().title(), "Categoria": categoria_limpa, "Local/Referência": registro["locais_destino"]["nome_exibicao"], "Cidade": registro["locais_destino"]["regiao_cidade"], "Dias": idade_dias, "Observação": registro.get("observacao_detalhe") or registro.get("detalhes_adicionais") or "", "SubSegmento": sub_seg, "Pegada": registro.get("pegada_digital") or f"anon_{registro['id']}", "Contato": registro.get("contato_aviso") or ""})
                if not dados_limpos:
                    dados_limpos = [
                        ...
                        {"ID": 994, "O que Falta": "Lavanderia Expressa Auto-Serviço", "Categoria": "Serviço Local / Novo Estabelecimento", "Local/Referência": "Avenida Das Palmeiras", "Cidade": "São Paulo/SP - Tatuapé", "Dias": 45, "Observação": "Prédios novos sem serviço.", "SubSegmento": "Investimento", "Pegada": "hash4", "Contato": ""},
                    ...
                    elif st.session_state.perfil_cliente == "investidor": dados_limpos = [d for d in dados_limpos if d["SubSegmento"] == "Investimento"]
                    ...
                    elif st.session_state.perfil_cliente == "jornalista": dados_limpos = [d for d in dados_limpos if d["SubSegmento"] in ["Zeladoria", "Investimento"]]
            ...
            if not df.empty:
                ...
                if not df_filtrado.empty:
                    ...
                    if espectador_analitico and st.session_state.perfil_cliente != "admin":
                        df_analitico = df_filtrado.groupby(["O que Falta", "Categoria", "Cidade"]).agg(Clientes_Unicos=("Pegada", "nunique"), Alertas_Totais=("ID", "count"), Maior_Espera=("Dias", "max")).sort_values(by="Clientes_Unicos", ascending=False).reset_index()
                        ...
                        for indice, dynamic_line in df_analitico.iterrows():
                            item_nome = dynamic_line['O que Falta']; clientes = int(dynamic_line['Clientes_Unicos']); alertas = int(dynamic_line['Alertas_Totais'])
                            classe_tag = "tag-calor-alta" if clientes >= 5 else ("tag-calor-media" if clientes >= 2 else "tag-calor-baixa")
                            label_tag = f"🔥 CRÍTICO • {clientes} CPFs Únicos" if clientes >= 5 else (f"⚠️ OPORTUNIDADE • {clientes} CPFs Únicos" if clientes >= 2 else f"🔹 INICIAL • {clientes} CPF Único")
                            st.markdown(f'<div class="bloco-lista-premium"><span class="{classe_tag}">{label_tag}</span><b style="color: #FFFFFF; font-size: 16px;">🏢 Falta: {item_nome}</b><div style="margin-top: 0.5rem; color: #aaaaaa; font-size: 13px;">⏱️ Demanda de {alertas} relatos • Espera: {dynamic_line["Maior_Espera"]} dias</div></div>', unsafe_allow_html=True)
                            detalhes_item = df_filtrado[df_filtrado['O que Falta'] == item_nome]
                            st.write("📍 **Localização das Reclamações Coletadas:**")
                            for loc in detalhes_item['Cidade'].unique(): st.markdown(f"  * **{loc}**")
                            st.markdown("<hr style='border-top: 1px dashed #333; margin: 1rem 0;'/>", unsafe_allow_html=True)
                    elif st.session_state.perfil_cliente != "admin":
                        df_agrupado = df_filtrado.groupby(["O que Falta", "Categoria", "Cidade"]).agg(Volume_Total=("ID", "count"), Menor_Idade=("Dias", "min"), Foco_Dono=("É_Minha_Loja", "max")).sort_values(by=["Foco_Dono", "Volume_Total"], ascending=[False, False]).reset_index()
                        ...
                        for indice, linha in df_agrupado.iterrows():
                            ...
                            classe_tag, label_tag = ("tag-calor-media", f"🎯 REVERSO • {int(volume)} Compradores") if "Marketplace" in filtro_frente else (("tag-calor-alta", f"🎯 SEU MERCADO • {int(volume)} Pedidos") if sou_alvo == 1 else ("tag-calor-baixa", f"🌍 CONCORRÊNCIA • {int(volume)} Pedidos"))
                            st.markdown(f'<div class="bloco-lista-premium"><span class="{classe_tag}">{label_tag}</span><b style="color: #FFFFFF; font-size: 16px;">📦 {item_nome}</b><div style="margin-top: 0.5rem; color: #aaaaaa; font-size: 13px;">⏱️ Alerta há {linha["Menor_Idade"]} dias</div></div>', unsafe_allow_html=True)
                            detalhes_item = df_filtrado[df_filtrado['O que Falta'] == item_nome]
                            for _, sub_linha in detalhes_item.iterrows():
                                ...
                                st.markdown(f"{f'🔥 **SEU ESTABELECIMENTO:** {sub_local}' if sub_local == loja_alvo_prioridade else f'📍 **Captado no concorrente:** {sub_local}'} ({sub_linha['Cidade']})")
                                if sub_linha['Observação']: st.info(f"💬 *Relato:* \"{sub_linha['Observação']}\"")
                                if contato_morador and ("Marketplace" in filtro_frente or sub_local != loja_alvo_prioridade): st.success(f"📱 **Cliente Faminto!** Chame no WhatsApp e ofereça o item: `{contato_morador}`")
                    ...
                    if st.session_state.perfil_cliente != "admin": st.info("ℹ️ Nenhum registro ativo encontrado.")
            else:
